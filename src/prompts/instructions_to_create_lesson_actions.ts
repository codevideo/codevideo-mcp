...
export const instructionsToCreateLessonActions = (additionalContext: string): string => {
    ...
    WORKFLOW TIP: After creating actions, you can store them using \`codevideo_set_current_actions\` and then use other tools like \`codevideo_validate_actions\`, \`codevideo_make_video\`, \`codevideo_make_blog_post\`, etc. without needing to re-specify the actions each time. These tools will automatically retrieve the stored actions if no actions parameter is provided.
    ...
    4. Call any other tools (make_video, make_blog_post, etc.) WITHOUT the actions parameter - they will use the stored actions automatically
    5. Use \`codevideo_get_current_actions\` at any later time to inspect what's currently stored if needed
