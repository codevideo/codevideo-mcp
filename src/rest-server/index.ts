...
class RestMcpServer {
  public app: express.Application; // Make app public
  ...
  private setupRoutes() {
    ...
    this.app.post('/codevideo-mcp', async (req, res) => {
      try {
        ...
        res.status(202).json({
          ...
          .header('Retry-After', '5'); // Suggest polling every 5 seconds
    ...
    // Get task status
    this.app.get('/codevideo-mcp/tasks/:taskId', (req, res) => {
    ...
    this.app.get('/codevideo-mcp/tasks/:taskId/result', (req, res) => {
      const { taskId } = req.params;
      const task = this.tasks.get(taskId);
    ...
    // Cancel a task
    this.app.delete('/codevideo-mcp/tasks/:taskId', (req, res) => {
      const { taskId } = req.params;
      const task = this.tasks.get(taskId);
    ...
    this.app.get('/codevideo-mcp/tasks', (req, res) => {
    ...
    this.app.get('/codevideo-mcp/capabilities', async (req, res) => {
      if (!this.mcpServer) {
      ...
      try {
        // You can expose available tools, prompts, and resources here
        res.json({
          tools: [
            'create_course_with_initial_metadata',
            'add_lesson_to_course',
            'add_actions_to_lesson',
            'make_video_from_actions',
            'make_video_from_lesson',
            'validate_actions',
            'translate_actions',
            // Add other tools from your MCP server
          ],
          prompts: [
            'instructions_to_create_a_course',
            'instructions_to_create_lesson_actions',
            'instructions_to_translate_actions'
          ],
          resources: [
            'action_names',
            'example_actions',
  ...
  private async processPrompt(taskId: string, prompt: string, args?: any): Promise<any> {
    ...
    // This is a basic implementation - you could use an LLM to do better parsing
    const lowerPrompt = prompt.toLowerCase();
    
    if (lowerPrompt.includes('create') && lowerPrompt.includes('course')) {
      return this.processTool(taskId, 'create_course_with_initial_metadata', {
        prompt,
        ...args
      });
    } else if (lowerPrompt.includes('video') && lowerPrompt.includes('lesson')) {
      return this.processTool(taskId, 'make_video_from_lesson', {
        prompt,
        ...args
      });
    } else if (lowerPrompt.includes('video') && lowerPrompt.includes('action')) {
      return this.processTool(taskId, 'make_video_from_actions', {
        prompt,
        ...args
      });
    } else if (lowerPrompt.includes('translate')) {
      return this.processTool(taskId, 'translate_actions', {
        prompt,
        ...args
      });
    } else if (lowerPrompt.includes('validate')) {
      return this.processTool(taskId, 'validate_actions', {
        prompt,
        ...args
      });
    } else {
      // Default to using the prompt as instructions for course creation
      return this.processTool(taskId, 'instructions_to_create_a_course', {
        prompt,
        ...args
      });
    }
  ...
  private async processTool(taskId: string, toolName: string, args?: any): Promise<any> {
    ...
    // In a real implementation, you'd use the MCP SDK to call tools
    ...
    // Mock result - replace with actual MCP tool execution
    return {
      tool: toolName,
      args,
      timestamp: new Date().toISOString(),
      // Add actual result data here
      data: `Processed ${toolName} with args: ${JSON.stringify(args)}`
    };
  }
